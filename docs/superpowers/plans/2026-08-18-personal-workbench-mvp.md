...
- Added a Vite development proxy whose target is always `127.0.0.1`, defaults to API port 3001, and accepts only an integer `LYJ_WORKBENCH_API_PORT` from 1 through 65535. A real-loopback integration test starts a controlled API plus Vite and requests `/api/health` through the Vite origin.
- Added `scripts/prepare-backup.ps1`. It targets only `\LYJWorkBench-OutboundCheckin`, disables future triggers, stops that exact task when running, re-queries and refuses a still-running/foreign task, and refuses backup while the configured `127.0.0.1` server port is listening. Controlled PowerShell checks exercised running, foreign-path, and live-listener cases without real task actions.
- Added a random per-launch non-secret instance token. The child echoes it only in `X-LYJ-Workbench-Instance` on health, and the launcher requires both `{ "status": "ok" }` and an exact header match before reporting ready or opening a browser. Direct health without a token retains the original JSON contract and no instance header.
- RED evidence captured before implementation: the Vite proxy test failed 10/10 (missing validated target helper and no forwarding); the tokenized health test lacked the header; and the collision test showed the incumbent responder could be accepted. The first backup RED run failed 3/3, but two failures also exposed a test-wrapper error policy issue; those wrappers were corrected before implementation.
- Sandbox-permitted checks after implementation passed: direct resolver default/override/rejection checks, PowerShell parse checks for both startup and backup scripts, controlled running-task disable/stop/re-query, foreign-path refusal, live-listener refusal, and `git diff --check`.
- **Controller verification:** `pnpm test` passed (127 server + 48 web), `pnpm build` passed, and `pnpm check` passed. The scheduler installer `-WhatIf` completed without changing task state. `pnpm local:start -NoOpen -Port 43131` served a 200 health response and SPA route plus a JSON 404 for an unknown API route, and exact-child cleanup left no listener. README examples now omit pnpm's extra `--`, which PowerShell interpreted as an ambiguous parameter.
