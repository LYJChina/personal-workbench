...
export function UpcomingRemindersCard({ api = sharedApi }: {
  ...
  return <section className="dashboard-card upcoming-reminders-card" role="region" aria-label="近期提醒">
    ...
    <div className="upcoming-scroll">{items.length === 0 ? <p className="empty-state">暂无提醒计划</p> : items.map((item) => <button type="button" key={item.id} onClick={() => navigate(`/reminders?selected=${encodeURIComponent(item.id)}`)}><span><strong>{item.name}</strong><small>{item.scheduleType === "workday" ? "法定工作日" : item.scheduleType === "daily" ? "每天" : item.scheduleType === "weekly" ? "每周" : item.scheduleType === "monthly" ? "每月" : "一次性"}</small></span><time>{item.nextRun ? new Date(item.nextRun).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", hour12: false }) : "待计算"}</time></button>)}</div>
  </section>;
}
