...
export interface SettingsApi extends PluginManagerApi, AiConnectionsApi, AccountSecurityApi {
  ...
  generateAiPersona?(): Promise<AiPersonaSettings>;
}
...
function AiPersonaPanel({ api }: { api: SettingsApi }) {
  ...
  async function save(event: FormEvent) { event.preventDefault(); if (!api.updateAiPersona || !value) return; const current = value; setMessage("保存中…"); try { setValue(await api.updateAiPersona({ assistantName: current.assistantName, personalityPrompt: current.personalityPrompt, profilePortrait: current.profilePortrait, systemPrompt: current.systemPrompt })); setMessage("AI 设置已保存"); } catch { setMessage("保存失败"); } }
  async function generate() { if (!api.generateAiPersona) return; setMessage("正在根据个人信息生成画像…"); try { setValue(await api.generateAiPersona()); setMessage("画像已生成，你可以继续修改后保存"); } catch { setMessage("画像生成失败，请检查模型连接"); } }
  return <section className="settings-section" aria-labelledby="ai-persona-heading"><div className="settings-section-heading"><div className="card-icon"><Icon name="sparkles" /></div><div><h3 id="ai-persona-heading">问问 AI 设置</h3><p>个人资料生成的画像、AI 性格和自定义提示词会合并注入每次对话。</p></div></div><form onSubmit={(event) => void save(event)}><div className="form-grid"><label>AI 名称<input value={value.assistantName} onChange={(event) => setValue({ ...value, assistantName: event.target.value })} /></label><label>AI 性格<input value={value.personalityPrompt} onChange={(event) => setValue({ ...value, personalityPrompt: event.target.value })} /></label></div><label>根据个人信息生成的画像<textarea rows={5} value={value.profilePortrait} onChange={(event) => setValue({ ...value, profilePortrait: event.target.value })} /></label><label>用户自定义系统提示词<textarea rows={6} value={value.systemPrompt} onChange={(event) => setValue({ ...value, systemPrompt: event.target.value })} /></label><div className="form-actions"><button className="button-secondary" type="button" onClick={() => void generate()}>根据个人信息重新生成</button><button className="button-primary" type="submit">保存并用于问问 AI</button>{message && <span role="status">{message}</span>}</div></form></section>;
}
