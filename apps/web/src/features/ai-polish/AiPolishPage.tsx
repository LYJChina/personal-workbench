  ...
  return <section className="daily-report-page ai-polish-page">
    ...
    <div className="daily-report-workspace">
      <form className="daily-report-input" onSubmit={generate}>
        ...
        <label><span>{preset.primaryLabel}<small>{preset.primaryHint}</small></span><textarea aria-label={preset.primaryLabel} placeholder={preset.primaryPlaceholder} value={primaryText} onChange={(event) => setPrimaryText(event.target.value)} rows={7} /></label>
        <label><span>{preset.secondaryLabel}<small>{preset.secondaryHint}</small></span><textarea aria-label={preset.secondaryLabel} placeholder={preset.secondaryPlaceholder} value={secondaryText} onChange={(event) => setSecondaryText(event.target.value)} rows={4} /></label>
        <details className="prompt-editor" open><summary>查看和修改提示词</summary><label>系统提示词<textarea aria-label="系统提示词" value={systemPrompt} onChange={(event) => setSystemPrompt(event.target.value)} rows={7} /></label><button className="button-ghost compact" type="button" onClick={() => setSystemPrompt(preset.systemPrompt)}>恢复此场景默认提示词</button></details>
